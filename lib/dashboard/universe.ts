...
export function dashboardUniverseTickers(): string[] {
  const out = new Set<string>(DASHBOARD_PULSE);
  for (const g of DASHBOARD_GROUPS) {
    out.add(g.benchmark);
    for (const t of g.tickers) out.add(t.t);
  }
